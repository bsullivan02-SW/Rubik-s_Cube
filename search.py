...
def uniformCostSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    frontier = util.PriorityQueueWithFunction(getCost) #look for the priority function in the tuple at position 2 (third element, for cost UCS)
    frontier.push((problem.getStartState(), list(), 0))
    return graphSearch(problem, frontier, nullHeuristic)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    "*** YOUR CODE HERE ***"#heuristic is a function for manhattan
    frontier = util.PriorityQueueWithFunction(getCost)  #lambda x is function    , x[2] is cost, x[0] is the state , 
    #Heuristic + cost to get there. g(n) <-- cost + h(n) <-- heuristic
    frontier.push((problem.getStartState(), list(), 0))
